...
def magazayi_bastan_basa_tara(manuel_mod=False, message_object=None):
    ...
    while True:
        ...
        for urun in urunler:
            try:
                ...
                # İlk otomatik kurulum bittiyse veya kullanıcı elle sorguluyorsa Telegram'a bas
    ...
    if not manuel_mod and not ilk_kurulum_bitti:
        ...
        bot.send_message(CHAT_ID, "✅ Bot başarıyla pusuya yattı! Amazon Depo hafızaya alındı, anlık indirimler ve yeni ürünler 30 dakikada bir otomatik taranacak.")

    if manuel_mod and yeni_bulunan_sayisi == 0:
        bot.send_message(hedef_chat, "✅ Tarama tamamlandı. Son tura göre yeni eklenen veya fiyatı düşen bir ürün şu an yok.")
...
if __name__ == "__main__":
    ...
    # 409 Çakışmalarına karşı zırhlandırılmış, kilitlenmeyen ana dinleme döngüsü
    while True:
        try:
            # allowed_updates=[] ve long_polling_timeout=1: Hayalet bot isteklerini anında düşürür!
            bot.polling(non_stop=True, timeout=10, long_polling_timeout=1, allowed_updates=[])
        except:
            # Eski bot saniyeler içinde düşeceği için, 4 saniye uyuyup döngüyü kesintisiz devam ettirir
            time.sleep(4)
